...
{
    /*0000*/	{(uint32_t*)& GeneralSettingsCAN.ID,        	  					{ROM, INTERNAL, DATA, BYTE, SINGLE, 	LENGHT_1, RES_WINDOW, RD_1, WR_2}},		//CAN ID
	...
	/*0004*/	{(uint32_t*)& GeneralSettingsCAN.baudRate,  	  					{ROM, INTERNAL, DATA, BYTE, SINGLE, 	LENGHT_2, RES_WINDOW, RD_1, WR_2}},		//CAN Speed
	/*0005*/	{(uint32_t*)& RS232_Settings.StopBits,  	  						{ROM, INTERNAL, DATA, BIT,  SINGLE, 		   0, RES_WINDOW, RD_1, WR_2}},		//-
	/*0006*/	{(uint32_t*)& NULL_PARAM,  											{ROM, INTERNAL, DATA, BYTE, SINGLE, 	LENGHT_1, RES_WINDOW, RD_1, WR_2}},		//RS232 SPEED
	/*0007*/	{(uint32_t*)& RS232_Settings.BaudRate,   							{ROM, INTERNAL, DATA, BYTE, SINGLE, 	LENGHT_2, RES_WINDOW, RD_1, WR_2}},		//RS232 Parity
	...
	/*0019*/	{(uint32_t*)& RS232_Settings.EnParity,       						{ROM, INTERNAL, DATA, BIT,  SINGLE, 	LENGHT_1, RES_WINDOW, RD_1, WR_2}},
	/*001A*/	{(uint32_t*)& RS232_Settings.BitParity,       						{ROM, INTERNAL, DATA, BIT,  SINGLE, 	LENGHT_1, RES_WINDOW, RD_1, WR_2}},
};
...
const ParamDescript ParamDescrTable_0xF0[] =                                                                         //<! Таблица параметров группы 0xF0xx
{
	...
	/*F01A*/	{(uint32_t*)& appProgramState,        											{ROM, INTERNAL, DATA, BYTE, SINGLE , LENGHT_1, RES_WINDOW, RD_1, WR_2}},
	...
	*/
	...
	for(uint32_t paramType = 0; paramType <= LAST_TYPE_PARAMETERS; paramType++)			// Цикл прохождения по типам жестких параметрам ParamDescrTable_ХХ
	{
 		...
 		for(int paramNumber = 0; paramNumber < ParametersTablesLenght[paramType]; paramNumber++) // Цикл прохождения по каждому параметру в таблице
		{
			...
			destinationAddress =  (uint8_t*)parameter.ParametersAddress;
			...
			sourceAddress = ((uint8_t*)&DeviceConstParameters[0]) + (fullParamNumber * 4);
...
void ParamSys::eraseSector(uint32_t sector)
{
	flashErasePage(sector);
	/*for(int i = 0; i < SECTOR_USED_NUMBER; i++)
	{
		if(erasedSectors[i] == SECTOR_USED_NUMBER)
		{
			return;
		}else
		{
		   erasedSectors[i] = sector;
		   erasedSectorsNumber++;
		   erase_sector(sector);
		   break;					
		}
...
{
//	TIM[2].On = 0;		// остановка таймера синхронихации памяти
//	TIM[2].Off = 1;
	...
	if(DeviceConstParametersDesyncFlag == true)
	{
    	...
    	EEPROM_ErasePage(DEVICE_PARAM_FLASH_PAGE_28, EEPROM_Main_Bank_Select);
		EEPROM_ErasePage(DEVICE_PARAM_FLASH_PAGE_29, EEPROM_Main_Bank_Select);
	}
	if(AppProgramDataDesyncFlag == true)
	{
		EEPROM_SetLatency(EEPROM_Latency_1);
    	EEPROM_ErasePage(APP_FLASH_PAGE_31, EEPROM_Main_Bank_Select);
	}
	if(AppProgramParametersDesyncFlag == true)
	{
		EEPROM_SetLatency(EEPROM_Latency_1);
    	EEPROM_ErasePage(APP_PARAM_FLASH_PAGE_30, EEPROM_Main_Bank_Select);
	}
	...
	if(AppProgramDataDesyncFlag == true)
	{
		...
		//EEPROM_ErasePage(APP_FLASH_PAGE_31, 0);
		//EEPROM_ErasePage(APP_FLASH_PAGE_31, 1);
		//EEPROM_ErasePage(APP_FLASH_PAGE_31, 2);
		//EEPROM_ErasePage(APP_FLASH_PAGE_31, 3);
	...
	if(AppProgramParametersDesyncFlag == true)
	{
		...
		//flashErasePage(APP_PARAM_FLASH_PAGE_30);
		//EEPROM_ErasePage(APP_PARAM_FLASH_PAGE_30, 0);
		//EEPROM_ErasePage(APP_PARAM_FLASH_PAGE_30, 1);
		//EEPROM_ErasePage(APP_PARAM_FLASH_PAGE_30, 2);
		//EEPROM_ErasePage(APP_PARAM_FLASH_PAGE_30, 3);
...
void SysTick_Handler(void)
{
    ...
    if (GetStatusSwTimer(&TIM[2]))
    {
		memorySyncInterruptHandler();
		TIM[2].On = 0;									// Запускаем таймер
		TIM[2].Off = 1; 
    } 
